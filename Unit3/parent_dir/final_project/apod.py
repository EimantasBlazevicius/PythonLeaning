...
def parse_command_line():
    ...
    parser.add_argument('-d', '--date', type=str, nargs=3)
    parser.add_argument('-s', '--surprise', action="store_true")
    parser.add_argument('-k', '--api_key', type=int, metavar='number')
...
def create_date(datelist, surprise):
    ...
    """
    start_date = datetime(1995, 0o6, 16)
    ...
    if (datelist and surprise) or (datelist and not surprise):
        ...
        try:
            random_time = start_date + timedelta(seconds=randint(0, int((datetime.now() - start_date).total_seconds())),)
            return random_time
        except ValueError:
            print("Can not create the random date")
...
def query_url(d, api_key):
    ...
    """
    date_reformed = d.strftime('%Y-%m-%d')
    if api_key:
        the_url = "https://api.nasa.gov/planetary/apod?api_key=" + str(api_key) + "&date=" + date_reformed
    else:
        the_url = "https://api.nasa.gov/planetary/apod?api_key=DEMO_KEY&date=" + date_reformed
    return the_url
...
def save_image(d, image):
    ...
    path = "/Users/eimantas.blazevicius/Documents/GitHub/PythonLeaning/Unit3/parent_dir/final_project"
    files = os.listdir(path)
    for name in files:
        if name == image_name:
            break
        else:
            ifile = open(image_name, 'wb')
            ifile.write(image)
            ifile.close
